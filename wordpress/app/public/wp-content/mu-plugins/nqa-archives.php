<?php
/**
 * Plugin Name: NQA – Archive & Taxonomy Listing Styling
 * Description: Brings the archive's colour-blocked visual language to the listing/archive
 *              views — the custom-post-type archives (Person / Organization / Event / Place),
 *              the Collections (`nqa_collection`) and Municipality taxonomy archives, plus
 *              category / tag / date archives. The query title becomes a colour-blocked header
 *              and each result renders as a catalogue card, matching the single-post "Archive
 *              details" panel and the Collections wayfinding grid. Pure CSS scoped to archive
 *              views (body.archive) — no template surgery, no child theme.
 * Version:     1.0.0
 *
...
add_action( 'wp_enqueue_scripts', function () {
	...
	wp_register_style( 'nqa-archives', false );
	wp_enqueue_style( 'nqa-archives' );

	// Pinned palette — same audited hexes as the Item Details card and the
	// Collections grid (WCAG 2.1 AA); never swapped for theme presets.
	$css = 'body.archive{'
			. '--nqa-violet:#503AA8;--nqa-yellow:#FFEE58;--nqa-pink:#F6CFF4;'
			. '--nqa-cream:#FBFAF3;--nqa-ink:#111;--nqa-base:#fff;'
	...
	wp_add_inline_style( 'nqa-archives', $css );
} );
